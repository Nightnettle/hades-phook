...
/*
void ragebot::DrawAngles(CInput::CUserCmd *pCmd, bool& bSendPacket)
{

	static Vector last_angle = Vector(0, 0, 0);
	static Vector last_angle2 = Vector(0, 0, 0);
	if (!bSendPacket)
		last_angle = pCmd->viewangles;

	if (bSendPacket)
		last_angle2 = pCmd->viewangles;

	string anglesx = "Pitch :" + std::to_string(pCmd->viewangles.x);
	string anglesy_real = "Real Yaw :" + std::to_string(last_angle.y);
	string anglesy_fake = "Fake Yaw :" + std::to_string(last_angle2.y);

	Color untrustedY_real =
		last_angle.y > 180
		|| last_angle.y < -180
		? Color(255, 0, 0, 255)
		: Color(255, 255, 255, 255);

	Color untrustedY_fake =
		last_angle2.y > 180
		|| last_angle2.y < -180
		? Color(255, 0, 0, 255)
		: Color(255, 255, 255, 255);
	...
	Color untrustedX =
		...
		? Color(255, 0, 0, 255)
		: Color(255, 255, 255, 255);
	g_Render->DrawString2(g_Render->font.ESP, 70, 425, untrustedX, FONT_LEFT, anglesx.c_str());
	g_Render->DrawString2(g_Render->font.ESP, 70, 450, untrustedY_real, FONT_LEFT, anglesy_real.c_str());
	g_Render->DrawString2(g_Render->font.ESP, 70, 450, untrustedY_fake, FONT_LEFT, anglesy_fake.c_str());
}
*/
...
void ragebot::DoAimbot(CInput::CUserCmd *pCmd, bool& bSendPacket)
{
	...
	if (TargetID >= 0 && pTarget)
	{
		...
		if (g_Options.Ragebot.PosAdj) pCmd->tick_count += TICKS_TO_TIME(g_Globals->interval_per_tick);
...
bool EdgeAntiAim(C_BaseEntity* pLocalBaseEntity, CInput::CUserCmd* cmd, float Wall, float Corner)
{
	Ray_t ray;
	trace_t tr;
	...
	for (float i = 0; i < 360; i++)
	{
		...
		if (tr.fraction != 1.0f)
		{
			...
			if ((leftTrace.fraction == 1.f) && (rightTrace.fraction != 1.f))
				vecDummy.y -= Corner; // left
			else if ((leftTrace.fraction != 1.f) && (rightTrace.fraction == 1.f))
				vecDummy.y += Corner; // right			

			cmd->viewangles.y = vecDummy.y;
			cmd->viewangles.y -= Wall;
...
void ragebot::DoAntiAim(CInput::CUserCmd *pCmd, bool& bSendPacket)
{
	...
	case 1: //sideways
	{
		...
		RealAngles.y = flip ? StartAngles.y - 90.f : StartAngles.y + 90.f;
	}
	break;
	case 2://slowspin
		RealAngles.y += static_cast<float>(yaw);
		break;
	case 3://fastspin
	{
		RealAngles.y = (float)(fmod(server_time / 0.05f * 360.0f, 360.0f));
	}
	break;
	case 4://backwards
	{
		g_Engine->GetViewAngles(StartAngles);
		StartAngles.y -= 180.f;
		RealAngles = StartAngles;
	}
	break;
	case 5:
	{
		RealAngles.y = pLocal->GetLowerBodyYaw();
	}
	break;
	case 6:
	{
		...
		if (NextLBYUpdate())
			RealAngles.y = StartAngles.y - 50;
		else
			RealAngles.y = StartAngles.y + 130;
	}
	break;
	case 7:
	{
		...
		RealAngles.y = flip ? StartAngles.y - 145.f : StartAngles.y + 145.f;
	}
	break;
	case 8:
	{
		...
		if (dir && pLocal->GetVelocity().Length2D() < 1)
		{
			...
			{
				RealAngles.y = StartAngles.y - 90;
			}
			else
			{
				RealAngles.y = StartAngles.y + 125;
				...
				{
					RealAngles.y = StartAngles.y + 145;
					jitterangle += 1;
				}
				else
				{
		...
		{
			RealAngles.y = StartAngles.y + 155;
	...
	break;
		...
		/*
		switch (g_Options.Misc.thirdpersontype)
		{
		case 0:
			pCmd->viewangles = RealAngles;
			break;
		case 1:
		...
		*/
	if (Globals::ySwitch) {
		Globals::FakeAngle = pCmd->viewangles.y;
	}
	else {
		Globals::RealAngle = pCmd->viewangles.y;
	}

	if (Globals::ySwitch && g_Options.Ragebot.YawTrue != 0)
		pCmd->viewangles.y = FakeAngles.y;
	if (!Globals::ySwitch && g_Options.Ragebot.YawFake != 0)
		pCmd->viewangles.y = RealAngles.y;
